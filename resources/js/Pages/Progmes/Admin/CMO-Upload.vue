<template>
    ...
    <content-container placeholder="Search CMO">
        ...
        <template v-slot:content-head>
            <div class="overflow-x-auto m-5 rounded shadow shadow-gray-400 bg-sky-800 pb-5">
                <div class="px-3 pt-3 font-bold text-white text-lg">
                    CMO Details
                </div>
                <div class="w-full flex items-center justify-between p-3">
                    <div class="w-full flex flex-col">
                        <div class="flex flex-row">
                            <div class="w-full mr-1 mb-1 flex flex-col">
                                <label for="discipline" class="font-bold text-white">Discipline</label>
                                <select id="discipline" v-model="form.discipline"
                                    class="py-1 px-3 h-10 text-sm border-none rounded">
                            ...
                            <div class="w-full mr-1 mb-1 flex flex-col">
                                <label for="program" class="font-bold text-white">Program</label>
                        ...
                        <div class="flex flex-row lg:mt-3">
                            <div class="w-full flex flex-col">
                                <label for="number" class=" font-bold text-white">CMO No.</label>
                                <input id="number" type="number" placeholder="CMO No." v-model="form.number"
                                    class="h-10 lg:mr-1 mt-1 rounded text-sm border-none">
                                <FormErrorMessage :message="$page.props.errors.number" />
                            </div>
                            <div class="w-full flex flex-col">
                                <label for="series" class=" font-bold text-white">Series</label>
                                <input id="series" type="number" placeholder="Series" v-model="form.series"
                                    class="h-10 lg:mr-1 mt-1 rounded text-sm border-none">
                            </div>
                            <div class="w-full flex flex-col">
                                <label for="version" class=" font-bold text-white">Version</label>
                    ...
                    <tr v-for="(item, index) in  areaArray " :key="index" :class="{ 'bg-slate-200': index % 2 == 1 }">
                        ...
                        <td v-if="index > 0" class="max-w-18 w-1/4 min-w-18 py-5 px-2 align-top">
                            <textarea ref="area" :id="'area' + index" v-model="form.area[index]"
                                class="areaInput min-h-max resize-none text-sm rounded w-full overflow-hidden border border-gray-400"
                                @input="handleAreaInput(index)"></textarea>
                        </td>
                        <td v-if="index > 0" class="min-w-18 py-5 px-2 align-top">
                            <textarea ref="minreq" :id="'minReq' + index" v-model="form.minReq[index]"
                                ...
                                @input="handleMinreqInput(index)"></textarea>
                        </td>
                    </tr>
                </template>
            </content-table>
        </template>
        <template v-slot:content-foot>
